...
contract Ownable {
    ...
    //EVENTS
    event ContractOwnerShipChangedTo(address ownerAddress);
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(
            newOwner != address(0),
            "Invalid address, please provide valid address to transfer ownership"
        );
        _setOwnership(newOwner);
    }

    function _setOwnership(address ownerAddress) private {
        _owner = ownerAddress;
...
contract Pausable is Ownable {
    ...
    //EVENTS
    event ContarctPausedBy(address ownerAddress);
    event ContarctResumedBy(address ownerAddress);

    function setContractOnPause() public onlyOwner whenNotPaused {
        _paused = true;
        emit ContarctPausedBy(msg.sender);
    }

    function setContractResume() public onlyOwner paused {
        _paused = false;
        emit ContarctResumedBy(msg.sender);
    }
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        // Given address to not be the owner of the tokenId
        require(
            address != _tokenOwner[tokenId],
            "Target approver address is same as owner."
        );

        //Check if caller of this function is owner of given token or token owner has approved caller of this function
        require(
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        //Check if token exist in smart conract
        require(__exists(tokenId), "Token does not exist in system");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(
            ...
            "Token with this id already exist in system"
        );
        require(
            to != address(0),
            "Provided address to mint the token is invalid"
        );

        // mint tokenId to given address & increase token count of owner
        _tokenOwner[tokenId] = to;
        _ownedTokensCount[to] = Counters.Counter.increase();
    ...
    function _transferFrom(
        ...
        require(
            ...
            "from address is not owner of this token"
        );

        // require token is being transfered to valid address
        require(to != address(0), "Trying to transfer to invalid address");

        // clear approval
        _clearApproval(tokenId);

        // update token counts & transfer ownership of the token ID
        _ownedTokensCount[from] = Counters.Counter.decrease();
